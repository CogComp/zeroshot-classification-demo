...
def convert_examples_to_features_bart_fever_rte(examples, label_list, max_seq_length,
                                 tokenizer, output_mode):
    """Loads a data file into a list of `InputBatch`s."""
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_ids = tokenizer.encode(example.text_a, example.text_b, max_length=max_seq_length,
                                     truncation='only_first')

        decode_tokens = tokenizer.decode(input_ids)
        # tokens = ["[CLS]"] + tokens_a + ["[SEP]"]
        # segment_ids = [0] * len(tokens)
        #
        # tokens += tokens_b + ["[SEP]"]
        ...
        #     logger.info("guid: %s" % (example.guid))
        #     logger.info("input_ids: %s" % " ".join([str(x) for x in input_ids]))
        #     logger.info("decode_tokens: %s" % "".join([str(x) for x in decode_tokens]))
        #     logger.info("input_mask: %s" % " ".join([str(x) for x in input_mask]))
        #     logger.info("label: %s (id = %d)" % (example.label, label_id))
...
def compute_bart_fever_rte_single_label(test_examples, model, tokenizer):
    parser = argparse.ArgumentParser()
    ...
    # test_examples = load_demo_input('fuck why my email not come yet', ['anger', 'this text expresses anger', 'the guy is very unhappy'])
    # test_examples = load_demo_input(premise_str, hypo_list.split(' | '))
    test_features = convert_examples_to_features_bart_fever_rte(
        test_examples, label_list, args.max_seq_length, tokenizer, output_mode)
    ...
    test_data = TensorDataset(test_all_input_ids, test_all_input_mask, test_all_label_ids)
    test_sampler = SequentialSampler(test_data)
    test_dataloader = DataLoader(test_data, sampler=test_sampler, batch_size=args.eval_batch_size)
    ...
    for input_ids, input_mask, label_ids in test_dataloader:
        ...
        with torch.no_grad():
            logits = model(input_ids=input_ids, attention_mask=input_mask, labels=None)
        logits = logits[0]
        if len(preds) == 0:
            preds.append(logits.detach().cpu().numpy())
        else:
...
def loading_bart_fever_rte_model(data_dir):
        bart_cache = {}
        for model_name in ["Bart-FEVER", "Bart-RTE"]:
            model, tokenizer = load_bart_fever_rte_model(model_name, data_dir)
            bart_cache[model_name] = (model, tokenizer)
        return bart_cache
